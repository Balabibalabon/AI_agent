...
class TestDocumentPathToMarkdown:
    ...
    def test_docx_returns_string(self) -> None:
        result = document_path_to_markdown(self.DOCX_FIXTURE)
        assert isinstance(result, str)
        assert len(result) > 0

    def test_pdf_returns_string(self) -> None:
        result = document_path_to_markdown(self.PDF_FIXTURE)
        assert isinstance(result, str)
        assert len(result) > 0

    def test_docx_contains_markdown_headings(self) -> None:
        result = document_path_to_markdown(self.DOCX_FIXTURE)
        assert "#" in result

    def test_docx_contains_known_content(self) -> None:
        result = document_path_to_markdown(self.DOCX_FIXTURE)
        assert "Model Context Protocol" in result

    def test_pdf_contains_known_content(self) -> None:
        result = document_path_to_markdown(self.PDF_FIXTURE)
        assert "Model Context Protocol" in result
